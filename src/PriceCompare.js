...
const PriceCompare = () => {
  ...
  Promise.all([
    ...
    if(parseFloat(data[0].data.amount) == parseFloat(data[2].USD)){
      setBTCName('You can purchase it on both platforms since the price is the same.')
    }
    ...
    if(parseFloat(data[1].data.rates.USD) == parseFloat(data[3].USD)){
      setETHName('You can purchase it on both platforms since the price is the same.')
    }
